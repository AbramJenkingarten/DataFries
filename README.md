...
```
Конвертируем время в секунды прошедшие с 10:00:00
Флаги Buy и Sell заменяем на b и s
Последовательность поступления заявок определяется полем OrderId.
Чем больше значение OrderId тем ближе к текущему моменту (младше) заявка.
Оставляем последнюю.
```
